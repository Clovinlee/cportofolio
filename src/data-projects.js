export var data_projects = [
    { 
        ...
        projects: [
            {
                name : "E Commerce Catalogue Mock Up using NestJS, GO, Spring and VUE",
                platform:"globe",
                ...
                backendTitle: ["JAVA BACKEND","GO BACKEND", "NESTJS BACKEND"],
                backend:["The first backend is made using Java with Spring as the main framework and JPA for the ORM. The backend was made using MVC-Repository pattern with DI (managed by spring). The backend uses criteria, and query builder to chain queries into advanced queries and filters. The backend also uses pageable with custom DTO to return similar response as the second backend.","The second backend is made using GoLang with Gin as the main framework and GORM as the ORM. The backend uses MVC-Repository pattern with DI practice. The backend uses query builder to achieve advanced search and filter, along with pagination as the response."," The third backend is made using NestJS with Prisma as the ORM and TS as the main language. The third backend (NestJS) has implemented authentication using JWT concept with refresh token. Other than that, the backend has same feature as the other two backend."],
